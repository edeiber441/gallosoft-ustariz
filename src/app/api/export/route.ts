...
export async function GET() {
  ...
  const { rows } = await sql`SELECT g.placa, g.candado, c.nombre AS criador, g.color,
    g.libras, g.onzas, g.cresta, g.patas, g.pico, g.mama, g.papa, g.marca_mes, g.marca_anio, g.creado_en
    FROM gallos g LEFT JOIN criadores c ON g.criador_id = c.id ORDER BY g.creado_en DESC`;

  const headers = ["Placa", "Candado", "Criador", "Color", "Libras", "Onzas", "Cresta", "Patas", "Pico", "Mama", "Papa", "Marca Mes", "Marca Año", "Fecha"];
  ...
  for (const row of rows) {
    ...
    csvLines.push([
      ...
      escape(record.mama),
      escape(record.papa),
      escape(record.marca_mes),
      escape(record.marca_anio),
